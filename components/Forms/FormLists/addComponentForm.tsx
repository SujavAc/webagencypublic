...
const FormComponent: React.FC = () => {
  const [componentData, setComponentData] = useState<DropdownData[]>([{ componentName: '', id: '' }]);
  ...
  const handleSubmit = () => {
    const fieldEmpty = (componentData.filter((data) => data.componentName === '' || data.id === '')?.length > 0);
    if(!fieldEmpty){
        console.log('Component values:', componentData);
    }
  };
